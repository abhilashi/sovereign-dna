    ...
    <div className="flex h-screen">
          ...
          {!claudeApiKey && (
            <div className="px-3 py-2 border-t border-border">
              <div className="flex gap-1">
                <input type="password" aria-label="Claude API key" value={apiKeyInput} onChange={e => setApiKeyInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleSaveApiKey(); }}
                ...
                <button onClick={handleSaveApiKey} aria-label="Save API key" disabled={!apiKeyInput.trim()} className="px-1.5 py-1 text-[9px] border border-border rounded-sm text-text-muted hover:text-accent disabled:opacity-30 transition-colors shrink-0">OK</button>
              </div>
            </div>
          )}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="border-t border-border px-4 py-2.5 flex items-center gap-2 shrink-0">
          ...
          <input ref={inputRef} aria-label="Ask about your genome" type="text" value={query} onChange={e => setQuery(e.target.value)} onKeyDown={handleKeyDown}
          ...
          <button onClick={handleSubmit} aria-label="Submit query" disabled={loading || streaming || !query.trim()}
