      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="border-t border-border px-4 py-2.5 flex items-center gap-2 shrink-0">
          ...
          <button onClick={handleSubmit} disabled={loading || streaming || !query.trim()}
            aria-label="Submit query" className="px-2.5 py-1 text-sm border border-border rounded-sm text-text-muted hover:text-accent hover:border-accent disabled:opacity-30 transition-colors">{'\u2192'}</button>
        </div>
      </div>
    </div>
  );
